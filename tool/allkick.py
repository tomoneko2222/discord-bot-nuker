...
async def on_ready():
    ...
    if guild:
        await kick_members_in_guild(guild)
    else:
        print("Invalid server ID or bot is not in the server.")
    await bot.close()

async def kick_members_in_guild(guild):
    tasks = []
    for member in guild.members:
        if member != guild.me:  # Don't kick the bot itself
...
async def kick_member(member):
    try:
        ...
        print(f"Cannot kick member: {member.name} (Permission Denied)")
